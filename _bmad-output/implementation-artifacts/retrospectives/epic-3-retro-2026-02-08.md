...
> **NOTE**: This document is a PRELIMINARY draft created before the interactive team retro.
> It will be replaced with the final version after the interactive retrospective session completes.
...
- E2E test framework established with 22 tests across 10 spec files
...
|---|-------------------|--------|----------|
| 1 | Eliminate "acceptable for MVP" filter | ✅ Completed | Zero instances of MVP-softened findings across all 21 Epic 3 deliverables |
| 2 | Code review presents findings before fixing | ✅ Completed | Every story shows findings table → user decision → fix/defer/dismiss |
| 3 | Report ALL metrics on every test run | ✅ Completed | Every story reports tests per project + lint errors + lint warnings |
| 4 | Quality gates are PASS/FAIL only | ✅ Completed | No "CONCERNS with deferred recommendations" anywhere in Epic 3 |

**All 4 process corrections from Epic 2 were maintained throughout Epic 3.** This is a genuine team improvement.
...
| 2 | Defense-in-depth tenantId in raw SQL | ⏳ In Progress | Rule added to project-context.md. Applied in new code. Legacy queries deferred to Epic 7 |
...
All 4 Epic 2 process corrections were maintained throughout the largest epic to date (21 deliverables). The MVP excuse was permanently banned. Quality gates operated at production standard consistently.
...
22 E2E tests across 3 epics catch an entirely different class of bugs: entity registration, route resolution, icon registration, guard execution order. This infrastructure is now available for all future epics.

### 6. User Testing Feedback Loop
Hands-on browser testing generated real user feedback that directly produced stories (3.10 file type presets, 4.7 test run preview) and bug fixes (20 Lucide icons, tenant_id mismatch, SET LOCAL parameterization).
...
### 4. Unit Tests Mock Everything
555+ tests passing while the app crashed on startup:
...
1. **Unit tests are necessary but not sufficient.** E2E tests catch entity registration, route resolution, icon registration, guard execution order — bugs that are invisible to mocked unit tests.
...
|---|--------|-------|------------------|
| 1 | **Enforce story size limits** — No story should produce >60 tests. Split if larger. | Bob (SM) | Zero oversized stories in Epic 4 |
| 2 | **Verify codebase before story creation** — Routes, interfaces, services, DTOs verified against actual code | Bob (SM) + Charlie (Dev) | Zero route/interface mismatches |
| 3 | **Browser smoke test for UI stories** — Don't rely solely on unit tests for frontend | erinc (PL) | Each UI story tested in browser before done |
| 4 | **Document guard ordering decisions** — Any new guard registration must specify execution order explicitly | Charlie (Dev) | All guards have documented ordering rationale |

### Technical Debt

| # | Item | Priority | Notes |
|---|------|----------|-------|
| 1 | Defense-in-depth tenantId in raw SQL queries | MEDIUM | Deferred to Epic 7 hardening sprint |
...
- Story 4-0 (BullMQ DLQ + idempotency) is FIRST — safety-critical for charged LLM operations
- Worker-engine service activation — needs BullMQ consumer setup
- LLM provider SDK integration (Mock + Google AI Studio + Vertex)
...
- Credit/billing system design (how to track, debit, enforce limits per tenant)
- Output validation strategies (verifying LLM output matches expected schema)
- Fan-out/fan-in patterns with BullMQ (parallel job execution + result aggregation)
- Token budget management (ensuring prompts don't exceed model context windows)

### Risks

- LLM API reliability (rate limits, timeouts, intermittent failures)
- Cost control (runaway loops burning API credits)
- Worker-engine service complexity (separate process, Redis-based communication)
...
| Process Health | PASS | All Epic 2 corrections maintained. New rules added. |
| Dependencies for Epic 4 | MET | All workflow definition infrastructure complete |
| Critical Path | Story 4-0 | BullMQ safety prerequisites must be first story |
| Significant Discoveries | None blocking | No epic updates required for Epic 4 |
...
2. **E2E gap assessment** — assess coverage gaps across all epics
3. **Begin Epic 4** — Story 4-0 (BullMQ safety prerequisites) first
4. **Apply new team agreements** — story sizing, browser testing, codebase verification

---

*Epic 3 delivered 21 story-level deliverables with 538 new tests (+133% growth), establishing the complete workflow definition layer. All 4 Epic 2 process corrections were maintained — the MVP excuse is permanently banned. New challenges emerged (story sizing, guard architecture, unit test limitations) and have been addressed with new rules and agreements. The team is well-positioned for Epic 4: all dependencies are met, the YAML schema contract is solid, and the E2E framework provides integration coverage. Epic 4 will bring the product to life — users will actually run workflows.*
